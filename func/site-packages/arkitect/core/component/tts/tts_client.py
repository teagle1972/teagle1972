...
class AsyncTTSClient(AsyncBaseTTSClient):
    _USAGE_HEADER_KEY = "X-Control-Require-Usage-Tokens-Return"
    _USAGE_HEADER_VALUE = "*"
    ...
    async def init(
        ...
        headers[self._USAGE_HEADER_KEY] = self._USAGE_HEADER_VALUE
    ...
    def _build_http_header(self) -> dict:
        headers = {
            ...
            self._USAGE_HEADER_KEY: self._USAGE_HEADER_VALUE,
        }
        return headers

    @staticmethod
    def _extract_usage_payload(payload_msg: Any) -> Optional[dict]:
        if not isinstance(payload_msg, dict):
            return None
        candidates = [
            payload_msg,
            payload_msg.get("payload_msg"),
            payload_msg.get("response_meta"),
            payload_msg.get("data"),
            payload_msg.get("result"),
        ]
    ...
    async def _get_tts_stream(
        ...
        while True:
            ...
            if response.session_finished:
                INFO(
                    "tts session_finished raw_payload session_id=%s payload=%s",
                    response.session_id,
                    response.payload_msg,
                )
                usage_payload = self._extract_usage_payload(response.payload_msg)
                yield TTSResponseChunk(event=response.event, usage=usage_payload)
                break
